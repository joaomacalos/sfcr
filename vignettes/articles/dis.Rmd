...
# Model DISequilibrium

As it is usual, we start by defining the equations, exogenous values, and paramenters of the model:
...
## One-shot increase in the costing margin
...
## Increase in the target inventories to sales ratio
...
dis2 <- sfcr_scenario(dis, list(shock2), 50)
```
...
expand_disinf_ext <- sfcr_expand(modify = disinf_ext,
                                 name = "Omega0",
                                 value = Omega0)

many_disinf <- sfcr_many_baseline(
  equations = disinf_eqs,
  expanded = expand_disinf_ext,
  initial = sfcr_set(p ~ 1, W ~ 1, UC ~ 1),
  periods = 100
  )
```
...
#exp_shock1 <- sfcr_expand(shock1, "Omega0", Omega0)

dsf1 <- sfcr_many_scenario(many_disinf, list(shock2), periods = 45)
```
...
## DISINF2: Increase in the real interest rate
...
## M.DISINF: The possibility of non-neutral inflation
...
```{r}
mdisinf_eqs <- disinf_eqs
mdisinf_eqs[[27]] <- c ~ alpha0 + alpha1 * yd + alpha2 * mh[-1]
...
```{r}
sfcr_dag_blocks_plot(mdisinf_eqs)
```
```{r}
mdisinf %>%
