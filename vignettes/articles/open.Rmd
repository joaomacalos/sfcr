...
```{r}
reg <- sfcr_sim(reg_eqs, reg_exg, reg_prms, 100, hidden = c("Hh" = "Hs"))
```
...
## Experiments with model REG

### An increase in the propensity to import of the South
...
### An increase in the government expenditures of the South
...
```{r}
shock2 <- sfcr_shock(
  variables = list(G_S ~ 25),
  start = 5,
  end = 60
)

reg_2 <- sfcr_scenario(reg, list(shock2), 60)
...
### An increase in the propensity to save of the Southern households
...
```{r}
shock3 <- sfcr_shock(
  variables = list(alpha1_S ~ 0.6),
  start = 5,
  end = 60
)

reg_3 <- sfcr_scenario(reg, list(shock3), periods = 60)
...
### A change in the liquidity preference of Southern households
...
```{r}
shock4 <- sfcr_shock(
  variables = list(lambda0_S ~ 1),
  start = 5,
  end = 60
)

reg_4 <- sfcr_scenario(reg, list(shock4), 60)
...
```{r}
open <- sfcr_sim(open_eqs, open_exg, open_prms, periods = 60, hidden = c("deltaor_S" = "deltaor_N"), .hidden_tol = 0.01)
open %>% select(period, deltaor_N, deltaor_S)
```
...
open_1 <- sfcr_scenario(open, scenario = list(shock1), periods = 60)
...
open2 <- sfcr_sim(open2_eqs, open2_exg, open2_prms, 60, initial = open2_initial, hidden = c("deltaor_N" = "deltaor_S"), .hidden_tol = 0.01)
...
```{r}
open2_1 <- sfcr_scenario(open2, list(shock1), 60)
...
## OPEN3: Adjustment through interest rates
...
open3 <- sfcr_sim(open3_eqs, open3_exg, open3_prms, periods = 100, initial = open3_initial,
...
open4 <- sfcr_sim(open4_eqs, open3_exg, open3_prms, periods = 50, initial = open3_initial,
                  hidden = c("deltaor_S" = "deltaor_N"), .hidden_tol = 0.01)

profvis::profvis(open4 <- sfcr_sim(open4_eqs, open3_exg, open3_prms, periods = 50, initial = open3_initial,
                  hidden = c("deltaor_S" = "deltaor_N"), .hidden_tol = 0.01))
...
open4_1 <- sfcr_scenario(open4, scenario = list(shock1), periods = 20)
...
open4_1l %>%
  ...
  geom_line(aes(linetype = name))
```
This model obviously explode and I had to decrease the number of periods to find some solution to it (as they do in the book).
...
```{r}
p1 <- sfcr_dag_blocks_plot(open4_eqs)
```


#### Making aggregate income dependent on interest rates
...
open4b_prms <- open3_prms

open4b_prms[[3]] <- alpha10_N ~ 0.7
open4b_prms[[4]] <- alpha10_S ~ 0.8
open4b_prms[[17]] <- iota_S ~ 0.5
open4b_prms[[18]] <- iota_N ~ 0.35

open4b <- sfcr_sim(open4b_eqs, open3_exg, open4b_prms, 100, initial = open3_initial, hidden = c("deltaor_S" = "deltaor_N"), .hidden_tol = 0.01)
...
### A perverse reaction function? The puzzling effect of interes rates revisited
...
open5 <- sfcr_sim(open5_eqs, open5_exg, open5_prms, periods = 100, initial = open3_initial,
                  hidden = c("deltaor_S" = "deltaor_N"), .hidden_tol = 0.01)
...
open5_1 <- sfcr_scenario(open5, scenario = list(shock1), periods = 60)
