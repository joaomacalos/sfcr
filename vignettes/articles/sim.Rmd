...
```{r}
hidden_equation <- c("H_h" = "H_s")
sim <- sfcr_sim(sim_eqs, sim_exg, sim_prms, 100, hidden = hidden_equation)
sim
```
...
sim %>%
  ...
  select(period, G_s, Y, TX_d, YD, H_s) %>%
  t() %>%
  round(digits = 0)
